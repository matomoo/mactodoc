...
import type { DataActivityLog, DataKpiStatistic4g } from "@/app/(project)/mdoc/def/interfaces";
...
export interface ChartVolteUserRef {
  getImageData: () => string | null;
}

interface ChartVolteUserProps {
  data: DataKpiStatistic4g[];
  dataActivityLog: DataActivityLog[];
}

const ChartVolteUser = forwardRef<ChartVolteUserRef, ChartVolteUserProps>(function ChartVolteUser(
  ...
  useEffect(() => {
    ...
    const rrcDataset = {
      ...
      data: uniqueDates.map((date) => {
        ...
        return (item?.volte_traffic ?? 0) as number;
      }),
      backgroundColor: chartJsColors[0],
    ...
    const dlUtilDataset = {
      ...
      data: uniqueDates.map((date) => {
        const item = sortedData.find((d) => d.begin_time === date);
        return (item?.volte_user ?? 0) as number;
      }),
