...
export function FilterBy_Date_NopKabupaten() {
  ...
  const {
    ...
    queryKey: ["ref-query-nop"],
    queryFn: async () => {
      const response = await fetch("/tinfra/api/meas-db-ti-sul/aggregate/ref-query-nop");
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();

      // Extract unique NOP names from response
      const rows = Array.isArray(data) ? data : data?.rows || [];
      const uniqueNops = Array.from(new Set(rows.map((item: { nop: string }) => item.nop))).map((name) => ({
  ...
  // Handler for NOP filter change (temporary state)
  const handleNopFilterChange = (nops: string[] | null) => {
    setTempNopFilter(nops);
  };

  // Toggle NOP selection (temporary state)
  ...
  // Update temporary NOP filter if Zustand store changes externally
  useEffect(() => {
    if (storeNop) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap">
        ...
        {storeNop && (
          <div className="flex items-center gap-2 text-gray-500 text-sm">
            ...
            <span className="rounded-full bg-blue-100 px-2 py-0.5 text-blue-800">NOP: {storeNop.toUpperCase()}</span>
            <Button
              variant="ghost"
              size="sm"
