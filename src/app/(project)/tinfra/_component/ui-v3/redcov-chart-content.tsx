...
interface IProps {
  ...
  redcovLevel: string;
  redcovLocation: string;
}

export default function RedcovChartContent({ redcovApiPath, redcovLevel }: IProps) {
  ...
  const {
    ...
    queryKey: ["redcov-data-achv", yearweek, redcovApiPath, redcovLevel, valueLocation],
    queryFn: async () => {
      if (!redcovApiPath || redcovApiPath === "noUrl") {
        return [];
      }

      const response = await fetch(
        [`${redcovApiPath}-achv?level=${viewBy}`, `valueLocation=${valueLocation}`, `yearweek=${yearweek}`].join("&"),
      );
      if (!response.ok) {
    ...
    enabled: !!(redcovApiPath && redcovApiPath !== "noUrl"),
  });

  const {
    data: redcovChartData,
    // isLoading:rciChartLoading,
    // error,
  } = useQuery({
    queryKey: ["redcov-data-chart", yearweek, redcovApiPath, redcovLevel, valueLocation],
    queryFn: async () => {
      if (!redcovApiPath || redcovApiPath === "noUrl") {
        return [];
      }
      ...
      const response = await fetch(
        [`${redcovApiPath}-chart?level=${viewBy}`, `&valueLocation=${valueLocation}`, `&yearweek=${yearweek}`].join(
          "&",
        ),
      );
      if (!response.ok) {
  ...
  const data = redcovData || [];
  const redcovChart = redcovChartData || [];

  // console.log("debug:", { data, redcovChart });
  ...
  const currentWeekIndex = redcovChart?.findIndex(
    (item: any) => item.yearweek === yearweek || item.yearweek === String(yearweek),
  );

  const lineChartData = {
    labels: redcovChart?.map((item: any) => item.yearweek) || [],
    datasets: [
      {
        label: "Redcov Percentage",
        data: redcovChart?.map((item: any) => parseFloat(item.red_optim_cells_pct)) || [],
        borderColor: "rgb(75, 192, 192)",
        backgroundColor: "rgba(75, 192, 192, 0.2)",
        tension: 0.3,

        // Per-point radius — current week gets bigger dot
        pointRadius: redcovChart?.map((_: any, i: number) => (i === currentWeekIndex ? 4 : 0)) || [],

        // Per-point color — current week gets accent color
        pointBackgroundColor:
          redcovChart?.map((_: any, i: number) =>
      ...
      {Object.entries(groupedData).map(([metric, techData]) => (
        <div key={metric} className="flex flex-row items-center lg:overflow-x-auto">
          ...
          <div className="col-span-2" style={{ width: "280px", height: "200px" }}>
            {redcovChart && redcovChart.length > 0 ? (
              <Line data={lineChartData} options={lineChartOptions} />
            ) : (
              <div className="flex h-full items-center justify-center">
                <p className="text-gray-500">No Redcov percentage data available</p>
              </div>
            )}
          </div>
        </div>
