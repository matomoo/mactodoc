...
type RemarkStatus = "Good" | "Degraded" | "Stable";
...
function getRemarkStyle(row: ComparisonRow): RemarkStyle {
  if (row.growth > 2 && row.delta > 0) {
    return { label: "Good", fillColor: "DCFCE7", textColor: "166534" }; // green-100 / green-800
  }
  if (row.growth > 2 && row.delta < 0) {
    return { label: "Degraded", fillColor: "FEE2E2", textColor: "991B1B" }; // red-100   / red-800
  }
  return { label: "Stable", fillColor: "FEF9C3", textColor: "854D0E" }; // yellow-100/ yellow-800
}
...
function addComparisonSlide(pres: PptxGenJS, rows: ComparisonRow[]): void {
  ...
  const baseCellLeft: PptxGenJS.TextPropsOptions = {
    ...baseCell,
    align: "left",
  };
  ...
  if (legendY < 5.3) {
    const legendItems: Array<{ fill: string; text: string; textColor: string }> = [
      { fill: "DCFCE7", text: "Good – growth >2% & delta positive", textColor: "166534" },
      { fill: "FEE2E2", text: "Degraded – growth >2% & delta negative", textColor: "991B1B" },
      { fill: "FEF9C3", text: "Stable – change within threshold", textColor: "854D0E" },
    ];
