...
import { extractBandFromCellName4G } from "../_function/helper";
...
export function useDataManagementProductivityAll({ data, rawDataSector }: UseDataManagementProps) {
  ...
  useEffect(() => {
    if (data?.rows && data.rows.length > 0) {
      ...
      const uniqueCells: string[] = Array.from(
        new Set(data?.rows.map((item: Data2G4GModel) => item.SITEID)),
      ).sort() as string[];
