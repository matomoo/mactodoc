...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await db_conn_v2.execute<Data2G4GModel>(sql`
        WITH params AS (
            SELECT '${sql.raw(searchByTgl2)}'::date AS selected_date  -- ✏️ change here only
        ),

        -- Daily aggregation for all dates needed
        daily AS (
            SELECT
                "Date"::date                        AS date,
        ...
        -- YoY: full year aggregation this year vs last year
        yoy AS (
            SELECT
                -- This Year (Jan 1 to selected_date)
                SUM(CASE
                ...
                -- Last Year (Jan 1 to same day last year)
                SUM(CASE
        ...
        -- MTD: 1st of month to selected date this year vs same period last year
        mtd AS (
            SELECT
        ...
        -- WoW: last 7 days vs prior 7 days
        wow AS (
            SELECT
                -- Current 7 days
                SUM(CASE
