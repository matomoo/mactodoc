...
export async function GET(request: Request) {
  ...
  // const sitaten = searchParams.get("kabupaten") || "---";
  // const saleId = searchParams.get("siteId") || "---";
  // const nop = searchParams.get("nop") || "---";
  // const kabupesCluster = searchParams.get("salesCluster") || "---";
  ...
  try {
    ...
    // const queryNop = `%${nop.toUpperCase()}%`;
    // const nopCondition = nop !== "All" ? sql`AND tref.siteid IN ${queryNop}` : sql``;
    ...
    if (multiSite === "---" || multiSite === "All" || multiSiteValues.length === 0) {
      ...
      multiSiteCondition = sql`AND tref.siteid = ${multiSiteValues[0].trim()}`;
    } else {
    ...
    } else if (aggregateBy === "G4_AGGRBY2") {
      // Use exact match for single cluster
      multiSiteCondition_aggregateBy = sql`tref.sector AS "G4_AGGRBY2",`;
    }
    ...
    if (aggregateBy === "G4_SITEID") {
      ...
      // Use exact match for single cluster
      multiSiteCondition_aggregateBy_grby = sql`t1."Begin Time",tref.sector`;
    }
