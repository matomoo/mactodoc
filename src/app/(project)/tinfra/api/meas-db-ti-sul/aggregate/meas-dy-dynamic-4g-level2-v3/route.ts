...
function buildCacheKey(params: URLSearchParams): string {
  ...
  const field2 = params.get("fieldToAggregate2") || "---";

  // Only include params that actually affect the SQL query
  return `meas-dy-4g:${field}:${field2}:${tgl1}:${tgl2}`;
}
...
async function runQuery(params: URLSearchParams) {
  ...
  const fieldToAggregate2 = params.get("fieldToAggregate2") || "---";
  ...
  return await db_conn_v2.transaction(async (tx) => {
    ...
    const result = await tx.execute(sql`
      ...
      SELECT
        ...
        tref.${sql.raw(fieldToAggregate2)}                    AS "G4_AGGRBY2",
      ...
      INNER JOIN ref_cell_4g tref
        ...
        ${searchByCondition}
      WHERE tref.remark IS NULL
      GROUP BY t1."Begin Time", tref.${sql.raw(fieldToAggregate)}, tref.${sql.raw(fieldToAggregate2)}
      ORDER BY t1."Begin Time", tref.${sql.raw(fieldToAggregate)}, tref.${sql.raw(fieldToAggregate2)}
    `);
